...
Zur Synthese von Einzellauten werden verschiedene Quelle-Filter-Modelle verwendet. Nachfolgend sind zwei Beispiele dargestellt. Einmal wird zur Realisierung eine Kaskadenstruktur verwendet, einmal eine Parallelstruktur.
...
Das Anregungssignal f\"ur die stimmhaften Laute soll den glottalen Luftstrom nachbilden. Dazu wurde die von Paul Taylor vorgeschlagene Formel benutzt %\cite[S. 331, Gleichung (11.30)]{TaylorTTS}:
\begin{equation}
\label{eq:sourcesignal}
u[n]=\begin{cases}
\frac{1}{2}(1-\cos(\pi n/N_1)) & 0\leq n\leq N_1\\
\cos(\pi(n-N_1)/(2N_2)) & N_1\leq n\leq N_2\\
0 & \mathrm{sonst}
\end{cases}
\end{equation}
...
Eine Filterstruktur nach Abbildung \ref{fig:qfm-vokal} wird zur Synthese von Vokallauten genutzt.

\begin{figure}[H]
	\centering
	\input{figures/Synthese/qfm-vokale.tex}
	\caption{Quelle-Filter-Modell f\"ur Vokallaute}
	\label{fig:qfm-vokal}
\end{figure}

Beispielhaft ist das Spektrum f\"ur den synthetisierten Vokal \enquote{i} in Abbildung \ref*{fig:spektr-synt-i} dargestellt.
...
Bei der Lautkombination werden einzelne Laute gefenstert und ineinander verschoben. Als Fensterfunktion wird ein Von-Hann-Fenster verwendet. Die Verschiebung zweier Fenster ist beispielhaft in Abbildung \ref{fig:kombination} abgebildet.

\begin{figure}[H]
	\centering
	\input{figures/Synthese/alpha1.tex}
